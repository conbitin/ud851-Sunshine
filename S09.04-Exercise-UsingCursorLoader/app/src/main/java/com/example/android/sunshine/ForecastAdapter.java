...
//      DONE (7) Generate a weather summary with the date, description, high and low
        ...
        if (mCursor.moveToPosition(position)) {
            Long date = mCursor.getLong(MainActivity.COLUMN_DATE);
            int id = mCursor.getInt(MainActivity.COLUMN_WEATHER_ID);
            double high = mCursor.getDouble(MainActivity.COLUMN_MAX_TEMP);
            double low = mCursor.getDouble(MainActivity.COLUMN_MIN_TEMP);
            String weatherForThisDay = SunshineDateUtils.getFriendlyDateString(mContext, date, true) + " - " +
                    SunshineWeatherUtils.getStringForWeatherCondition(mContext, id) + " - " +
                    SunshineWeatherUtils.formatHighLows(mContext, high, low);
